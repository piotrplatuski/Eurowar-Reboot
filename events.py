...
class Event:
    ...
    def fill_in_text(self, countries):
        result = []
        text = self.get_text().copy()
        countries = list(countries)
        if self.get_amount_countries() == 1:
            person1 = MimPerson(countries[0].get_languages().pop())
            ...
            text = str.replace(text, '<ruler1>', countries[0].get_ruler().get_full_name())
        ...
        if self.get_amount_countries() == 2:
            ...
            text = str.replace(text, '<ruler2>', countries[1].get_ruler().get_full_name())
